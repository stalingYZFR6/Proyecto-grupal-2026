...
const Login = () => {
  ...
  const iniciarSesion = async () => {
    try {
      ...
      // Fetch user record to get role and employee id
      ...
      const { rol, id_empleado } = userRecord;
      // Store in localStorage
      localStorage.setItem("usuario-supabase", JSON.stringify({
        email: usuario,
        rol,
        id_empleado,
      }));
    ...
    <div className="vh-100 d-flex align-items-center justify-content-center overflow-hidden" style={{ background: "var(--bg-main)" }}>
      <Container fluid className="p-0 h-100">
        <Row className="g-0 h-100">
          ...
          <Col lg={7} className="d-none d-lg-flex align-items-center justify-content-center position-relative" style={{ background: "linear-gradient(135deg, #0f172a 0%, #1e293b 100%)", overflow: "hidden" }}>
            <div className="position-absolute w-100 h-100" style={{ opacity: 0.1 }}>
              <div className="position-absolute" style={{ top: "-10%", left: "-10%", width: "40%", height: "40%", borderRadius: "50%", background: "white", filter: "blur(100px)" }}>
              </div>
            </div>
            <div className="position-absolute" style={{ bottom: "-10%", right: "-10%", width: "40%", height: "40%", borderRadius: "50%", background: "#38bdf8", filter: "blur(100px)" }}>
            </div>
          </Col>
          ...
          <Col lg={5} xs={12} className="d-flex align-items-center justify-content-center" style={{ background: "var(--bg-card)" }}>
            <div className="w-100 px-4 px-md-5" style={{ maxWidth: "480px" }}>
              ...
              />
              <p className="text-center mt-5 text-premium-muted small">&copy; 2026 AssisTech Enterprise. Todos los derechos reservados.</p>
            </div>
        </Row>
      </Container>
    </div>
